...
#define GO_REQARG	"%s: Option '%s' ben”tigt ein Argument"
#define GO_REQARG2	"%s: Option '-%c' ben”tigt ein Argument"
...
int getopt (int argc, char **argv, const char *optstring)
{
...

  {
	...
	if (opterr != 0)
	  {
	    if (c < 040 || c >= 0177)
	      fprintf(stderr,  GO_ILLOPT2 "\n", argv[0], c);
	    else
	      fprintf(stderr,  GO_ILLOPT3 "\n",
		       argv[0], c);
	  }
	return '?';
      }
